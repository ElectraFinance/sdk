...
export const isolatedOrderUpdateSchema = z
  .object({
    I: z.string(), // id
    A: z.number(), // settled amount
...
export const isolatedFullOrderSchema = z
  .object({
    I: z.string(), // id
    O: z.string(), // sender (owner)
    P: z.string().toUpperCase(), // asset pair
...
const updateMessageSchema = baseAddressUpdate.extend({
  ...
  o: z
    .tuple([isolatedFullOrderSchema.or(isolatedOrderUpdateSchema)])
    .optional(),
});
...
const initialMessageSchema = baseAddressUpdate.extend({
  ...
  b: balancesSchema,
  o: z.array(isolatedFullOrderSchema).optional(), // When no orders — no field
});
